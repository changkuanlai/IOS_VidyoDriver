...
/*! Configuration of video layout resolution and frame rate. //视频布局分辨率和帧速率的配置。
...
/*! Call state indicates the client's state and whether it is engaged in a call.
	- Used for the following requests:  // 呼叫状态显示客户机的状态以及是否从事打电话。——用于以下请求:
...
typedef struct VidyoClientInEventMute_
{
	/*! VIDYO_TRUE to mute local media device for specified in event, VIDYO_FALSE to un-mute.
		...
		and #VIDYO_CLIENT_IN_EVENT_MUTE_VIDEO to (un-) .
	*/
	VidyoBool willMute;
...
typedef struct VidyoClientInEventLayout_
{
	/*!
		...
		allowed values 0~1.
     //在视频中显示较大尺寸区域的远程参与者的数目
     布局。值0使所有远程参与者以相等的大小显示.
     区域.一个值1导致一个远程参与者所谓的“首选”的参与者，显示在
     视频布局中比其他参与者更大的区域。范围
     允许值0 1。
	*/
...
typedef struct VidyoClientInEventSetFlag_
{
	/*!
     ...
     vidyo_true设置对应于指定的事件标志，vidyo_false明确标志。
     使用vidyo_client_in_event_set_echo_detect启用/禁用回波检测，
     vidyo_client_in_event_set_echo_cancel启用/禁用声学回声消除，
     vidyo_client_in_event_set_audio_agc启用/禁用自动增益控制，
     和vidyo_client_in_event_set_freeze_image启用/禁用视频图像冻结。
	*/
...
typedef struct VidyoClientInEventSetOrientation_
{
	/*!
		...
		correct value for this parameter. //定位自己的视频和/或视频布局设置。通常,这个值跟踪相机的位置在设备上相对设备的正常取向。相机位于设备前将“通常”VIDYO_CLIENT_ORIENTATION_UP的取向。并不是所有的设备坚持这个计划,需要一些实验获得正确的为这个参数值。
	*/

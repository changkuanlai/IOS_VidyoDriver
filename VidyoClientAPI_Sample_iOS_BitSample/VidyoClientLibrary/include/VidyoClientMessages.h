...
typedef enum VidyoClientInEvent_
{
	...
	*/
	...
	/*!
		Change how own video shown in video layout. //改变自己的视频所示视频布局。
	...
	*/
	...
	/*!
		Stop sharing local application window in conference. //在会议上分享本地应用程序窗口
	...
	*/
	...
	/*!
		...
		Control number and order of monitors. //进入全屏模式。控制数量和顺序的监视器。
	...
	*/
	...
	/*!
		Called by application to login to user to Portal //由应用程序调用用户登录到门户

		@see Corresponding parameter structure #VidyoClientInEventLogIn
	*/
	VIDYO_CLIENT_IN_EVENT_LOGIN = 1600,
	/*!
		Called by application to cancel login process.  // 由应用程序调用取消登录过程。
	...
	*/
	...
	/*!
		Called by GUI to join to Conference via Room Link with location tag as an extra parameter //调用GUI加入会议通过房间与位置标记作为一个额外的参数
	
		@see corresponding parameter structure #VidyoClientInEventRoomLinkEx
	*/
	VIDYO_CLIENT_IN_EVENT_ROOM_LINK_EX = 1606,
	/*!
		Used to control connection with conference manager //用于控制与会议有关的经理

		@see Corresponding parameter structure #VidyoClientInEventSetOffline
	*/
	VIDYO_CLIENT_IN_EVENT_SET_OFFLINE = 1700,
	/*!
		Used to freeze/unfreeze the image from the video capturer //用于冻结/解冻的图像视频捕获者

		@see Corresponding parameter structure #VidyoClientInEventSetFlag
	*/
	VIDYO_CLIENT_IN_EVENT_SET_FREEZE_IMAGE = 1701,
	/*!
		Used to set orientation of own video and/or video layout. //用于设置定位自己的视频和/或视频布局。
	...
	*/
	...
	/*!
		Reports that conference is interrupted by incoming call //报道说,会议由来电打断
	...
	*/
	...
	/*!
     Identify the location of the font file to use  //确定要使用的字体文件的位置
	...
	*/
	...
	/*!
     ...
     为了对下一次会议产生影响。
     按钮栏默认启用，如果
     事件永远不会发送。发送此事件将导致相应的# vidyo_client_out_event_enabled_button_bar来
     确认设置。
	...
	*/
	...
	/*!
     Used to start or stop video sources in local renderer //用于启动或停止局部渲染视频源
...
typedef enum  VidyoClientOutEvent_
{
	...
	*/
	...
	/*!
		Acknowledge change of how own video shown in video layout.  //承认改变的自己的视频所示视频布局。
	...
	*/
	...
	/*!
		Indicates that remote application window share added. //表明,远程应用程序窗口共享补充道。
	...
	*/
	...
	/*!
		Resize of video layout. //调整视频的布局。
	...
	*/
	...
	/*!
     ...
     be sent. //确认状态栏部件的启用状态在视频布局上的更改。
     发送相应的# vidyo_client_out_event_enabled_status_bar事件将导致这个事件
     被发送。
...
typedef enum VidyoClientRequest_
{
	...
	*/
	...
	/*!
		Get all configuration data. //获得所有配置数据。
	...
	*/
	...
	/*!
		Set writable configuration data. //设置可写的配置数据。
  ...
  // 设置文本标签的视频布局,根据语言设置。
	...
	*/
	...
	/*!
		Get the current call state (see documentation for VidyoClientCallState)//获取当前呼叫状态(参见文档VidyoClientCallState)
	...
	*/
	...
	/*!
		Set region rectangle for video layout. //为视频布局设置矩形区域。
	...
	/*!
		...
		Can not be set during a call.//设置渲染窗口的父。零意味着端点不指定父渲染窗口。渲染窗口仍将被创建不能设置在打电话。
